...
nc = ["r" if ox.simplification._is_endpoint(G2, node) else "y" for node in G2.nodes()]
...
ani = FuncAnimation(fig, animate,init_func=init , frames=32, interval = 100)
plt.show()
#%%
fig, ax = ox.plot_graph_routes(G, routes, node_size = 0, show=False, close=False)
route0 = list(routes)[0]
...
for i in range(len(route0)):
    ax.scatter(G.nodes[route0[i]]['x'] , G.nodes[route0[i]]['y']
                , s=100,
               c='y', alpha=0.5, edgecolor='none', zorder=4)
    plt.show()
# %%
